...
def save_to_csv_plain(self, image_name, prompt1, extracted_text1, prompt2, extracted_text2, stay_duration):
        csv_file = 'experiment_data_plain_gpt_tiktok.csv'
        print("Saving to CSV:", os.path.abspath(csv_file))
        fieldnames = ['Image Name', 'Prompt',"Answer","Reason", 'Stay Duration', 'Platform', 'Model Name']
        file_exists = os.path.isfile(csv_file)
        current_time = str(datetime.datetime.now().time())[:-7]
        with open(csv_file, 'a', newline='', encoding='utf-8') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
...
class Plain_bot:
    ...
    def experiment(self, n):
        ...
        for i in range(n):
            ...
            prompt1, extracted_text1, prompt2, extracted_text2 = self.model.fundamental_expor(screen_shot_name)
            stay_duration = 0
            ...
            save_to_csv_plain(self, screen_shot_name[45:], prompt1, extracted_text1, prompt2, extracted_text2, stay_duration)
